-- Migration 007: Multi-School Role Hierarchy
-- Adds: schools, school_registration_requests, registration_codes, school_years,
--       subjects, teachers, students, audit_logs
-- Updates: profiles (role hierarchy), classes (school FK)

-- ============================================
-- 1. SCHOOLS (replaces school_settings for multi-school)
-- ============================================
...
-- ============================================
-- 2. SCHOOL REGISTRATION REQUESTS
-- ============================================
...
-- ============================================
-- 3. REGISTRATION CODES (invitation codes per school)
-- ============================================
...
-- ============================================
-- 4. UPDATE PROFILES (add school_id, new role hierarchy)
-- ============================================
...
-- Migrate old roles to new hierarchy FIRST (before constraint change)
...
-- Update role CHECK constraint to new hierarchy
...
-- ============================================
-- 6. UPDATE CLASSES (add school_id FK, school_year_id, grade_level)
-- ============================================
...
-- ============================================
-- 8. TEACHERS (extension of profiles)
-- ============================================
...
-- ============================================
-- 9. STUDENTS (extension of profiles)
-- ============================================
...
CREATE OR REPLACE FUNCTION auth._is_role(required_role TEXT)
RETURNS BOOLEAN AS $$
    SELECT EXISTS (
...
CREATE OR REPLACE FUNCTION auth._user_school_id()
RETURNS UUID AS $$
...
-- SCHOOLS
-- --------------------------------------------------
-- super_admin: full access. admin_sekolah: read own school. guru/murid: read own school.
CREATE POLICY "schools_super_admin_all" ON schools
    FOR ALL USING (auth._is_role('super_admin'))
    WITH CHECK (auth._is_role('super_admin'));

CREATE POLICY "schools_admin_read_own" ON schools
    FOR SELECT USING (
        auth._is_role('admin_sekolah') AND id = auth._user_school_id()
    );

CREATE POLICY "schools_guru_murid_read_own" ON schools
    FOR SELECT USING (
        (auth._is_role('guru') OR auth._is_role('murid')) AND id = auth._user_school_id()
    );

-- --------------------------------------------------
-- SCHOOL REGISTRATION REQUESTS
-- --------------------------------------------------
-- Anyone can submit a request (insert). Only super_admin can update/delete.
CREATE POLICY "reg_req_insert_public" ON school_registration_requests
    FOR INSERT WITH CHECK (true);

CREATE POLICY "reg_req_select_public" ON school_registration_requests
...
-- REGISTRATION CODES
-- --------------------------------------------------
-- admin_sekolah: manage codes for own school. super_admin: all.
...
CREATE POLICY "reg_codes_admin_select_own" ON registration_codes
    FOR SELECT USING (
        auth._is_role('admin_sekolah') AND school_id = auth._user_school_id()
    );
...
-- Code can be read during registration (anon, no auth needed)
...
CREATE POLICY "sy_guru_murid_read" ON school_years
    FOR SELECT USING (
        (auth._is_role('guru') OR auth._is_role('murid')) AND school_id = auth._user_school_id()
    );

-- --------------------------------------------------
-- CLASSES
-- --------------------------------------------------
-- Recreate policies for multi-school
...
CREATE POLICY "classes_teacher_update" ON classes
    FOR UPDATE USING (
        auth._is_role('guru') AND teacher_id = auth.uid()
    );
...
CREATE POLICY "teachers_guru_read_own" ON teachers
    FOR SELECT USING (
        auth._is_role('guru') AND id = auth.uid()
    );
...
CREATE POLICY "students_guru_select_own" ON students
    FOR SELECT USING (
        auth._is_role('guru') AND school_id = auth._user_school_id()
    );

CREATE POLICY "students_murid_read_own" ON students
    FOR SELECT USING (
        auth._is_role('murid') AND id = auth.uid()
    );

-- --------------------------------------------------
-- AUDIT LOGS (insert-only via service key; select by super_admin)
-- --------------------------------------------------
CREATE POLICY "audit_insert_service" ON audit_logs
    FOR INSERT WITH CHECK (true);

CREATE POLICY "audit_select_super_admin" ON audit_logs
    FOR SELECT USING (auth._is_role('super_admin'));

-- --------------------------------------------------
-- PROFILES (update policies for new hierarchy)
-- --------------------------------------------------
-- Recreate profile policies with school scope
...
-- admin_sekolah can read profiles in their school
CREATE POLICY "profiles_select_admin_own_school" ON profiles
    FOR SELECT USING (
        auth._is_role('admin_sekolah') AND school_id = auth._user_school_id()
    );

-- guru can read student profiles in their school
CREATE POLICY "profiles_select_guru" ON profiles
    FOR SELECT USING (
        auth._is_role('guru') AND (role = 'murid' OR id = auth.uid()) AND school_id = auth._user_school_id()
    );
...
-- admin_sekolah can update profiles in their school (except role)
...
-- Insert for new user registration (trigger-based)
CREATE POLICY "profiles_insert_trigger" ON profiles
    FOR INSERT WITH CHECK (auth.uid() = id);

-- --------------------------------------------------
-- EXAMS (update for multi-school scope)
-- --------------------------------------------------
-- Add school_id to exams
...
-- Recreate exam policies for new role hierarchy
...
CREATE POLICY "exams_select_guru" ON exams
    FOR SELECT USING (
        (auth._is_role('guru') OR auth._is_role('admin_sekolah')) AND teacher_id = auth.uid()
    );

CREATE POLICY "exams_select_super_admin" ON exams
    FOR SELECT USING (auth._is_role('super_admin'));
...
CREATE POLICY "exams_update_guru" ON exams
...
CREATE POLICY "exams_delete_guru" ON exams
...
-- --------------------------------------------------
-- SUBMISSIONS (update for admin_sekolah access)
-- --------------------------------------------------
...
CREATE POLICY "submissions_select_admin_sekolah" ON submissions
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM exams e JOIN profiles p ON p.id = auth.uid()
                WHERE e.id = exam_id AND p.role = 'admin_sekolah' AND e.school_id = p.school_id)
    );

CREATE POLICY "submissions_select_super_admin" ON submissions
    FOR SELECT USING (auth._is_role('super_admin'));

-- Students can only see their own published submissions
...
CREATE POLICY "submissions_update_admin_sekolah" ON submissions
    FOR UPDATE USING (
        EXISTS (SELECT 1 FROM exams e WHERE e.id = exam_id AND e.school_id = auth._user_school_id())
        AND auth._is_role('admin_sekolah')
    );

-- --------------------------------------------------
-- EXAM ACCESS CODES (update policies for new roles)
-- --------------------------------------------------
...
-- --------------------------------------------------
-- VIOLATION LOGS (update policies for new roles)
-- --------------------------------------------------
...
-- --------------------------------------------------
-- ANALYTICS CACHE (update policies for new roles)
-- --------------------------------------------------
...
-- Auto-create profile on signup (updated for multi-school)
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO profiles (id, full_name, role, school_id, nisn, nis, status)
    VALUES (
        NEW.id,
...
-- Auto-create extension records when profile role is set
CREATE OR REPLACE FUNCTION handle_profile_role_change()
RETURNS TRIGGER AS $$
BEGIN
    -- When role changes to guru, ensure teachers row exists
    ...
    END IF;
    -- When role changes to murid, ensure students row exists  
...
-- Increment registration code use_count
...
-- Active school year for each school (max 1 per school)
...
-- ============================================
-- SEED: Migrate school_settings -> schools (if only one school)
-- ============================================
...
-- Assign existing profiles to the default school
UPDATE profiles SET school_id = '00000000-0000-0000-0000-000000000001'::UUID
WHERE school_id IS NULL;

-- Update classes with default school
UPDATE classes SET school_id = '00000000-0000-0000-0000-000000000001'::UUID
WHERE school_id IS NULL;
...
-- SELECT * FROM schools;
-- SELECT id, email, role, school_id, status FROM profiles ORDER BY role;
