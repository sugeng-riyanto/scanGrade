...
# `claim(key, seconds)` used to live here: one atomic Redis `SET NX EX`, with a
# per-process table inside it for the case where Redis is unreachable. It had one
# caller — the `sync-draft` throttle — which now derives the answer from the row it
# writes (`app/routes/api.py::_check_rate_limit`), so the function was deleted
# rather than kept: a leftover claim is a second clock waiting to be reused, and
# the per-process table inside it was the very thing it was written to remove.
